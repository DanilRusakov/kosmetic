  ...
  function getPriceHtmls($prices, $non_standard_theme='') {
    ...
    $custom_templates = $this->getCustomTemplates();
		...
		if ( !empty($custom_templates) ) {
			...
			$data['text_price'] = $text_price;
			$data['text_tax'] = $text_tax;
			$data['text_discount'] = $text_discount;
			$data['text_points'] = $text_points;
			$data['text_reward'] = $text_reward;
			$data['text_stock'] = $text_stock;
			$data['text_minimum'] = $text_minimum;
			$data['text_manufacturer'] = $text_manufacturer;
			
			$results = array();
			foreach ( $custom_templates as $tpl_key => $tpl_route ) {
				$results[$tpl_key] = $this->loadTemplate($tpl_route, $data);
			}
			return $results;
			
		} elseif ( $non_standard_theme == 'mstore' ) {
      ...
      */
      ...
      if (!$special) {
			...
			if ($discounts) {
				$html .= '<ul class="discount">';
        foreach ($discounts as $discount) {
          $html .= '<li>'.$discount['quantity'].''.$text_discount.'<span>'.$discount['price'].'</span></li> ';
        }
				$html .= '</ul>';
			}
			
			if ($points) {
				$html.= '<div class="points">'.$text_points.' <span>'.$points.'</span></div> ';
			}
			if ($reward) {
				$html.= '<div class="reward">'.$text_reward.' <span>'.$reward.'</span></div> ';
			}
    ...
    // OPC080178 constitute theme
      ...
      */  
      ...
      if ($product_info['quantity'] <= 0) {
			...
			if ( $price_val > 0 || $product_settings['zakaz_price_null']) {
				if (!$special) {
					$html4.= '<span class="price_no_format'.$product_id.'">'.$price.'</span>';
					$html4.= '<meta itemprop="price" content="'.$price_val.'">';
				} else {
					$html4.= '<span class="price-new"><span class="price_no_format'.$product['product_id'].'">'.$special.'</span></span>';
					$html4.= '<meta itemprop="price" content="'.$special_val.'">';
				}
				$html4.= '<meta itemprop="priceCurrency" content="'.$this->config->get('config_currency').'">';
			}
      ...
      if (!$special) {
		...
		}	elseif ($theme_name == 'pixelshop') {

			//$html = "<ul class=\"list-unstyled\">";
			if (!$special) {
				$html .= "<span class=\"price-default bk_price\">".$price."</span>";
			} else {
				$html .= " <span class=\"price-old\">".$price."</span></span>";
				$html .= "<span class=\"price-new bk_price\">".$special."<span class=\"procent-skidka\"> %</span></span>";
			}
			if ($tax) {
				$html .= "<span class=\"price-tax\">".$text_tax." ".$tax."</span>";
			}
			
		} elseif ( $theme_name == 'oct_luxury') {
			
			$html.= '<ul class="list-unstyled price-ul">';
			
			if (!$special) {
				$html.= '<li class="row"> ';
				$html.= '<h2 class="price col-sm-12" id="main-product-price">'.$price.'</h2> ';
				$html.= '</li> ';
			} else {
				$html.= '<li class="row"> ';
				$html.= '<h2 class="price col-sm-6" id="main-product-special">'.$special.'</h2> ';
				$html.= '<p class="col-sm-6 price-old" style="text-decoration: line-through;" id="main-product-price">'.$price.'</p> ';
				$html.= '</li> ';
			}
			
			$html.= '<li class="row">';
			
			$html.= '<p class="col-sm-6 found-cheaper"><a data-effect="mfp-zoom-out" onclick="get_oct_popup_found_cheaper('.$product_id.');">Нашли дешевле?</a> ';
			
			if ($tax) {
				$html.= '<p class="col-sm-6 product-tax-box">'.$text_tax.' <span id="main-product-tax">'.$tax.'</span></p>';
			}
			$html.= '</li> ';
			
			if ($points) {
				$html.= ' <li class="row">';
				$html.= ' <p class="col-sm-12">'.$text_points.''.$points.'</p>';
				$html.= ' </li>';
			}
			if ($discounts) {
				foreach ($discounts as $discount) {
					$html.= '<li class="row">';
					$html.= '<p class="col-sm-12">'.$discount['quantity'].$text_discount.$discount['price'].'</p> ';
					$html.= '</li>';
				}
			}
		...
		} elseif ($theme_name == 'theme661') {  // Sweets Store
