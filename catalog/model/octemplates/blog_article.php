...
class ModelOctemplatesBlogArticle extends Model {
	...
	public function getArticles($data = array()) {
		...
		// 	$filters = explode(',', $data['filter_category_id']);

		// 	foreach ($filters as $filter_id) {
		// 		$implode[] = (int)$filter_id;
		// 	}

		// 	$sql .= " AND a2c.oct_blog_category_id IN (" . implode(',', $implode) . ")";
		// }

		// if (isset($data['filter_tag']) && !empty($data['filter_tag'])) {
		// 	$sql .= " AND ad.tag LIKE '%" . $this->db->escape($data['filter_tag']) . "%'";
		// }
	...
	public function getTotalArticles($data = array()) {
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'ad.name_asc') {
				$sql .= " ORDER BY LCASE(" . $data['sort'] . ") ASC";
			} elseif ($data['sort'] == 'ad.name_desc') {
				$sql .= " ORDER BY LCASE(" . $data['sort'] . ") DESC";
			} elseif ($data['sort'] == 'a.sort_order_asc') {
