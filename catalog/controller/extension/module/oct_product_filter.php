...
class ControllerExtensionModuleOctProductFilter extends Controller {
  ...
  public function index() {
    ...
    'manufacturer_logic_between_manufacturers' => $oct_product_filter_data['manufacturer_logic_between_manufacturers'],
    ...
    if ($oct_product_filter_data['stock_status']) {
      $stock_array = array('in_stock', 'out_of_stock');
    ...
    $data['custom_sort'][] = array(
    'name'    => $this->language->get('text_sort_'.str_replace('|', '_', strtolower(str_replace(array('p.', 'pd.', 'order='), '', $oct_product_filter_data['default_sort'])))),
    ...
    foreach ($oct_product_filter_data['custom_sort'] as $custom_sort) {
      ...
      $custom_sort_mod_deep =  strtolower(str_replace(array('p.', 'pd.', 'order='), '', $custom_sort));
    ...
    $data['url'] = urldecode($this->url->link($route, $this->global_url));
  ...
  public function filterProducts() {
    ...
    $language_keys = array('button_popup_view','button_cart','text_model','text_stock');
    ...
    $json['url'] = urldecode($this->url->link($route, $this->global_url));
    ...
    if (!isset($this->request->get['update_seo_url'])) {
      ...
      if ($results) {
        foreach ($results as $result) {
          if ($result) {
            ...
            $oct_advanced_options_settings_data = $this->config->get('oct_images_by_options_data');
      ...
      $pagination->url = urldecode($this->url->link($route, $this->global_url . '&page={page}'));
  ...
  public function sitemap() {
    ...
    if ($oct_product_filter_status && isset($oct_product_filter_data['enable_seo']) && $oct_product_filter_data['enable_seo']) {
      ...
      if ($results) {
        ...
        foreach ($results as $result) {
          $output .= '<url>';
          $output .= '	<loc>'.HTTP_SERVER.$result['seo_url'].'</loc>';
