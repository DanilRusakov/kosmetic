<?php
class ControllerExtensionModuleOctFilterProducts extends Controller {
	public function index() {
		...
		$this->document->addScript('catalog/view/javascript/octemplates/filter_products/jquery-ui-slider-pips.js');
		$this->document->addStyle('catalog/view/javascript/octemplates/filter_products/jquery-ui-slider-pips.css');
		$this->document->addStyle('https://code.jquery.com/ui/1.10.4/themes/flick/jquery-ui.css');

		$this->load->model('catalog/product');
		$this->load->model('extension/module/oct_filter_products');
		$this->load->model('localisation/currency');
		$this->load->language('extension/module/oct_filter_products');
		...
		if (isset($this->request->get['route'])) {
			if ($this->request->get['route'] == 'product/category') {
				$data['products_data'] = $this->model_extension_module_oct_filter_products->getProductsByCategoryData($data['category_id']);
				$data['page_type'] = 'category';
			} elseif ($this->request->get['route'] == 'product/manufacturer/info') {
				$data['products_data'] = $this->model_extension_module_oct_filter_products->getProductsByManufacturerData($data['manufacturer_id'], false);
				$data['page_type'] = 'manufacturer';
			}
		} 
	
		$data['oct_filter_products_data'] = $this->config->get('oct_filter_products_data');
			...
			foreach ($attributes as $attribute) {
				...
				if (isset($attribute_value[1])) {
					...
					foreach ($attribute_vals as $val) {
						$data['a_vals'][] = md5($attribute_value[0].':'.$val);
					}
				}
			}
		}
		...
		if ($data['products_data']) {
			...
			if (isset($this->request->get['route'])) {
				if ($this->request->get['route'] == 'product/category' && isset($oct_filter_products_data['product_categories_array']) && in_array($data['category_id'], $oct_filter_products_data['product_categories_array'])) {
					return $this->load->view('extension/module/oct_filter_products', $data);
				} elseif ($this->request->get['route'] == 'product/manufacturer/info') {
					return $this->load->view('extension/module/oct_filter_products', $data);
				}
			} 
			...
			return $this->load->view('extension/module/oct_filter_products_empty', $data);
		}
	}
	...
	public function getProductsByCategory() {
	    ...
	    // popup_view end
			...
			$results = $this->model_extension_module_oct_filter_products->getProducts($data);
        ...
        foreach ($this->model_catalog_product->getProductOptions($result['product_id']) as $option) {
          ...
          'economy'     => $economy,
			...
			if ($category_info) {
				...
				$product_total = $this->model_extension_module_oct_filter_products->getTotalProductsByCategory($data);
			...
			$products_data = $this->model_extension_module_oct_filter_products->getProductsByCategoryData($this->request->post['category_id']);
						...
						foreach($attribute_value['attr_values'] as $value) {
							if (isset($av_implode[$attribute_value_id])) {
								if ($av_implode && in_array($value, $av_implode[$attribute_value_id])) {
									$attribute_values[] = array(
										'name'  					=> $value,
										'attribute_value' => md5($attribute_value_id.':'.$value)
									);
								}
							}
						}
        ...
        foreach ($this->model_catalog_product->getProductOptions($result['product_id']) as $option) {
          ...
          'economy'     => $economy,
			...
			if ($manufacturer_info) {
				...
				$product_total = $this->model_extension_module_oct_filter_products->getTotalProductsByManufacturer($data);
			...
			$products_data = $this->model_extension_module_oct_filter_products->getProductsByManufacturerData($this->request->post['manufacturer_id'], false);
